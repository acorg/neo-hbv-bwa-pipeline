...
# 'out' must have the identical value set in bwa-aln-l.sh
out=$task-hbv.fastq.gz

echo "02-bwa-aln-1 sbatch.sh running at `date`" >> $log
...
then
    ...
    then
        ...
        # Request an exclusive machine because bwa-aln-l.sh  will tell bwa and
